...
{
    public class ByteArrayJournalDao<TJournalPayload> : BaseByteArrayJournalDao<TJournalPayload>
    {
        public ByteArrayJournalDao(
            IAdvancedScheduler scheduler,
            IMaterializer mat,
            AkkaPersistenceDataConnectionFactory<TJournalPayload> connection,
            JournalConfig<TJournalPayload> journalConfig,
        ...
        public async Task InitializeTables(CancellationToken token)
        {
            ...
            await connection.CreateTableAsync<JournalRow<TJournalPayload>>(TableOptions.CreateIfNotExists, journalFooter, token);
